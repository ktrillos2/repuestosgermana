...
import { trackContactInteraction, GA_CONVERSION_ID } from "@/lib/conversion-utils"
...
export function ContactSection({ data, settings }: ContactProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      // Track conversion for form submission
  ...
  const contactItems = [
    {
      ...
      details: settings?.phoneNumber
        ? [`(+57) ${settings.phoneNumber}`, ...(settings.whatsappNumber ? [`(+57) ${settings.whatsappNumber}`] : [])]
        : ["(+57) 302 545 9865"],
      color: "#0066B1",
      href: settings?.phoneNumber ? `tel:+57${settings.phoneNumber}` : "tel:+573025459865",
    },
    {
      id: "email",
      icon: Mail,
      title: data?.emailTitle || "Email",
      details: settings?.email ? [settings.email] : ["info@repuestosgermana.com"],
      color: "#c41e3a",
      href: settings?.email ? `mailto:${settings.email}` : "mailto:info@repuestosgermana.com",
    },
    {
      id: "schedule",
      icon: Clock,
      title: data?.scheduleTitle || "Horario",
      details: settings?.schedule || ["Lun - Vie: 8:00 - 17:00", "Sábado: 8:30 - 15:00"],
      color: "#0066B1",
    },
    {
      id: "location",
      ...
      details: settings?.address ? [settings.address] : ["Bogotá, Colombia"],
      color: "#c41e3a",
    },
  ]
                  ...
                  <div className="bg-[#111111] border border-[#1a1a1a] rounded-sm p-6 hover:border-[#0066B1]/50 transition-all duration-300 group hover:scale-105 h-full">
                    {info.href ? (
                      <a
                        href={info.href}
                        className="block"
                        onClick={(e) => trackContactInteraction(e, info.href || "", `Contact Section ${info.title}`)}
                      >
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4">
                  ...
                  >
                    <a
                      ...
                      onClick={(e) => trackContactInteraction(e, `https://wa.me/57${settings?.whatsappNumber || "3025459865"}`, "Contact Section WhatsApp CTA")}
                    >
